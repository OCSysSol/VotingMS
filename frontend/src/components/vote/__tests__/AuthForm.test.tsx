...
describe("AuthForm", () => {
  ...
  it("renders lot number and email inputs", () => {
    render(
      <AuthForm onSubmit={() => {}} isLoading={false} />
    );
  ...
  it("calls onSubmit with lot number and email", async () => {
    ...
    render(
      <AuthForm onSubmit={onSubmit} isLoading={false} />
    );
  ...
  it("shows Verifying... and disables button when loading", () => {
    render(
      <AuthForm onSubmit={() => {}} isLoading={true} />
    );
    expect(screen.getByRole("button", { name: "Verifying..." })).toBeDisabled();
  });

  it("Continue button is enabled immediately on render (no loading gate)", () => {
    render(
      <AuthForm onSubmit={() => {}} isLoading={false} />
    );
  ...
  it("does not call onSubmit when loading and submitted again", async () => {
    ...
    const onSubmit = vi.fn();
    render(
      <AuthForm onSubmit={onSubmit} isLoading={true} />
    );
