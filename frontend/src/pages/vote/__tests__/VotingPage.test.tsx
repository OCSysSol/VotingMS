...
describe("VotingPage", () => {
  ...
  it("progress bar updates on selection", async () => {
    ...
    const yesButtons = await waitFor(() => screen.getAllByRole("button", { name: /^Vote For — / }));
  ...
  it("in-arrear single lot: shows all-in-arrear banner but general motions remain fully interactive", async () => {
    ...
    const forButtons = screen.getAllByRole("button", { name: /^Vote For — / });
  ...
  it("regression: submit request includes inline votes with actual choices (not abstained)", async () => {
    ...
    const yesButtons = screen.getAllByRole("button", { name: /^Vote For — / });
    const noButtons = screen.getAllByRole("button", { name: /^Vote Against — / });
  ...
  it("choices seeded for locked motion (all selected lots voted)", async () => {
    ...
    await waitFor(() => {
      const forButtons = screen.getAllByRole("button", { name: /^Vote For — / });
      const againstButtons = screen.getAllByRole("button", { name: /^Vote Against — / });
